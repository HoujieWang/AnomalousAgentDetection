...
The code implements a Bayesian modeling pipeline for large-scale agent mobility data on dynamic networks, including data cleaning, spatial discretization and clustering, group-level flow modeling, and individual-level regime-shift detection using dynamic Bayesian models. Note that the code files are costomized to the Knoxville data generated by collaborators from STR https://str.us, not sharable to public.
...
- The knoxville is generated by STR https://str.us, not sharable to public, the code files above mainly serve as example file to run this method.
---
